...
#define STEP_MOTOR_PULSE_BY_DELAY           false                   //使能由阻塞带动的脉冲发生(测试用,不建议实际应用中开启)
...
typedef struct 
{
    ...
    struct
    {
        ...
        stepMotorENALevel       ENA_Level;      //有效控制电平
...
typedef struct
{
    ...
    struct 
    {
        ...
        /*控制角度*/
        uint32_t                pulse;           //要发送的脉冲个数
        uint32_t                pulse_cnt;       //已发出的脉冲个数
